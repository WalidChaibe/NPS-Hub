        ...
        try:
            ...
            if run:
                ...
                def slide_1():
                    ...
                    donut_chart(axes[0],df[(df["Year"]==selected_year)&(df["Month"]==selected_month)]["Complaint_Category"].value_counts())
                    donut_chart(axes[1],df[(df["Year"]==selected_year)&(df["Month"]<=selected_month)]["Complaint_Category"].value_counts())
                ...
                show_fig(slide_1())
                slides_for_pdf.append({"title":"FINAL - Total Complaints Issued","fig":slide_1()})

                # Slide 2 - Lead Time
                st.subheader("Lead Time")
                def slide_2():
                    tmp=df.copy()
                    tmp["First_dt"]=pd.to_datetime(tmp["First Approval Date"],errors="coerce")
                    tmp["Final_dt"]=pd.to_datetime(tmp["Final Approval Date"],errors="coerce")
                    g=tmp.groupby("Ref NB",as_index=False).agg(First_dt=("First_dt","min"),Final_dt=("Final_dt","max"))
                    g["LeadTime_days"]=(g["Final_dt"]-g["First_dt"]).dt.total_seconds()/86400.0
                    g["Year"]=g["Final_dt"].dt.year; g["Month"]=g["Final_dt"].dt.month
                    g=g.dropna(subset=["Year","Month","LeadTime_days"]); g=g[g["LeadTime_days"]>=0]
                    def mavg(yr): return g[g["Year"]==yr].groupby("Month")["LeadTime_days"].mean().reindex(months_range)
                    def ytda(yr):
                        d=g[(g["Year"]==yr)&(g["Month"]<=selected_month)]
                        return float(d["LeadTime_days"].mean()) if len(d) else np.nan
                    cats=MONTH_LABELS[:selected_month]+["AVG-YTD"]
                ...
                show_fig(slide_2())
                slides_for_pdf.append({"title":"FINAL - Lead Time (Days)","fig":slide_2()})

                # Slide 3 - Valid Quality Count
                ...
                def slide_3():
                    ...
                    def ytdc(yr): return int(base[(base["Year"]==yr)&(base["Month"]<=selected_month)].shape[0])
                    ...
                    add_simple_value_labels(ax,b1,lambda v:f"{int(v)}",1)
                    add_simple_value_labels(ax,b2,lambda v:f"{int(v)}",1)
                    ...
                    plt.tight_layout(rect=[0,0.05,1,1]); return fig
                show_fig(slide_3())
                slides_for_pdf.append({"title":"FINAL - Valid Quality Count","fig":slide_3()})

                # Slide 4 - Quality Defects CM vs YTD
                def slide_4(top_n=15):
                    ...
                    yp=base[(base["Year"]==prev_year)&(base["Month"]<=selected_month)]["Defect"].value_counts()
                    ys=base[(base["Year"]==selected_year)&(base["Month"]<=selected_month)]["Defect"].value_counts()
                    all_d=pd.Index(ys.index).union(yp.index).union(cs.index).union(cp.index)
                    summ=pd.DataFrame({"CM_prev":cp.reindex(all_d,fill_value=0),"CM_sel":cs.reindex(all_d,fill_value=0),
                                       "YTD_prev":yp.reindex(all_d,fill_value=0),"YTD_sel":ys.reindex(all_d,fill_value=0)})
                ...
                show_fig(slide_4(TOPN_QUALITY_DEFECT))
                slides_for_pdf.append({"title":"FINAL - Quality Defects (CM vs YTD)","fig":slide_4(TOPN_QUALITY_DEFECT)})

                # Slide 5 - Quality Defects Current Month
                def slide_5(top_n=10):
                    base=df[(df["Is_Valid"]==True)&(df["Complaint_Category"]=="Quality")].copy()
                    base["Defect"]=base["Reason"].astype(str).str.strip()
                    ...
                    ax.set_xticklabels([fill(str(s),18) for s in counts.index],fontsize=10,rotation=25,ha="right")
                    ax.set_ylim(0,max(1,counts.max())*1.15)
                    ax.spines["top"].set_visible(False); ax.spines["right"].set_visible(False); ax.grid(False)
                    plt.tight_layout(); return fig
                show_fig(slide_5(TOPN_QUALITY_CM))
                slides_for_pdf.append({"title":"FINAL - Quality Defects (Current Month)","fig":slide_5(TOPN_QUALITY_CM)})

                # Slide 6 - Quality Cost
                def slide_6(top_n=15):
                    ...
                    yp=base[(base["Year"]==prev_year)&(base["Month"]<=selected_month)].groupby("Defect")["Cost Amount"].sum()
                    ys=base[(base["Year"]==selected_year)&(base["Month"]<=selected_month)].groupby("Defect")["Cost Amount"].sum()
                    ...
                    fig.subplots_adjust(bottom=0.35); return fig
                try:
                    show_fig(slide_6(TOPN_COST_DEFECT))
                    slides_for_pdf.append({"title":"FINAL - Quality Cost (Credit Note)","fig":slide_6(TOPN_COST_DEFECT)})
                except Exception as e:
                    st.warning(f"Cost slide skipped: {e}")

                # Slide S1 - Service Count
                ...
                def slide_s1():
                    ...
                    plt.tight_layout(rect=[0,0.05,1,1]); return fig
                show_fig(slide_s1())
                slides_for_pdf.append({"title":"FINAL - Valid Service Count","fig":slide_s1()})

                # Slide S2 - Service Reasons CM vs YTD
                def slide_s2(top_n=15):
                    ...
                    yp=base[(base["Year"]==prev_year)&(base["Month"]<=selected_month)]["Reason_Svc"].value_counts()
                    ys=base[(base["Year"]==selected_year)&(base["Month"]<=selected_month)]["Reason_Svc"].value_counts()
                    all_r=pd.Index(ys.index).union(yp.index).union(cs.index).union(cp.index)
                    summ=pd.DataFrame({"CM_prev":cp.reindex(all_r,fill_value=0),"CM_sel":cs.reindex(all_r,fill_value=0),
                                       "YTD_prev":yp.reindex(all_r,fill_value=0),"YTD_sel":ys.reindex(all_r,fill_value=0)})
                    summ=summ.sort_values(["YTD_sel","CM_sel"],ascending=False)
                    if top_n: summ=summ.head(int(top_n))
                    cmp=pd.to_datetime(f"{prev_year}-{selected_month:02d}-01").strftime("%b-%y")
                    cms=pd.to_datetime(f"{selected_year}-{selected_month:02d}-01").strftime("%b-%y")
                    ...
                    ax.legend(loc="upper center",bbox_to_anchor=(0.5,-0.22),ncol=4,frameon=False,fontsize=10)
                    fig.subplots_adjust(bottom=0.35); return fig
                show_fig(slide_s2(TOPN_SERVICE_REASON))
                slides_for_pdf.append({"title":"FINAL - Service Reasons (CM vs YTD)","fig":slide_s2(TOPN_SERVICE_REASON)})

                # Slide S3 - Service Reasons Current Month
                def slide_s3(top_n=10):
                    base=df[(df["Is_Valid"]==True)&(df["Complaint_Category"]=="Service")].copy()
                    ...
                    add_simple_value_labels(ax,bars,lambda v:f"{int(v)}",0.6)
                    ax.set_xticks(np.arange(len(counts)))
                    ax.set_xticklabels([fill(str(s),22) for s in counts.index],fontsize=10,rotation=30,ha="right")
                    ax.set_ylim(0,max(1,counts.max())*1.15)
                    ax.spines["top"].set_visible(False); ax.spines["right"].set_visible(False); ax.grid(False)
                    plt.tight_layout(); return fig
                show_fig(slide_s3(TOPN_SERVICE_CM))
                slides_for_pdf.append({"title":"FINAL - Service Reasons (Current Month)","fig":slide_s3(TOPN_SERVICE_CM)})
                ...
                def slide_i1():
                    fig,axes=plt.subplots(1,2,figsize=(13.33,7.5),dpi=300)
                    donut_chart(axes[0],df_issued[(df_issued["Year"]==selected_year)&(df_issued["Month"]==selected_month)]["Complaint_Category"].value_counts())
                    donut_chart(axes[1],df_issued[(df_issued["Year"]==selected_year)&(df_issued["Month"]<=selected_month)]["Complaint_Category"].value_counts())
                    ml=pd.to_datetime(f"{selected_year}-{selected_month:02d}-01").strftime("%b-%y")
                    axes[0].text(0.5,-0.10,f"ISSUED – {ml}",transform=axes[0].transAxes,ha="center",va="top",fontsize=13,color="#4D4D4D")
                    axes[1].text(0.5,-0.10,f"ISSUED – {selected_year} YTD",transform=axes[1].transAxes,ha="center",va="top",fontsize=13,color="#4D4D4D")
                    fig.legend(["Service","Quality","Invalid"],loc="lower center",ncol=3,frameon=False,prop={"size":14})
                    plt.tight_layout(rect=[0,0.08,1,1]); return fig
                show_fig(slide_i1())
                slides_for_pdf.append({"title":"ISSUED - Total Complaints","fig":slide_i1()})
                ...
                def slide_i2():
                    ...
                    plt.tight_layout(rect=[0,0.05,1,1]); return fig
                show_fig(slide_i2())
                slides_for_pdf.append({"title":"ISSUED - Valid Count","fig":slide_i2()})

                st.subheader("Quality (Current Month)")
                def slide_iq(top_n=12):
                    ...
                    add_simple_value_labels(ax,bars,lambda v:f"{int(v)}",0.6)
                    ax.set_xticks(np.arange(len(counts)))
                    ax.set_xticklabels([fill(str(s),28) for s in counts.index],fontsize=10,rotation=35,ha="right")
                    ax.set_ylim(0,max(1,counts.max())*1.15)
                    ax.spines["top"].set_visible(False); ax.spines["right"].set_visible(False); ax.grid(False)
                    plt.tight_layout(); return fig
                show_fig(slide_iq(TOPN_QUALITY_CM))
                slides_for_pdf.append({"title":"ISSUED - Quality Reasons (Current Month)","fig":slide_iq(TOPN_QUALITY_CM)})

                st.subheader("Service (Current Month)")
                def slide_is(top_n=10):
                    ...
                    ax.spines["top"].set_visible(False); ax.spines["right"].set_visible(False); ax.grid(False)
                    plt.tight_layout(); return fig
                show_fig(slide_is(TOPN_SERVICE_CM))
                slides_for_pdf.append({"title":"ISSUED - Service Reasons (Current Month)","fig":slide_is(TOPN_SERVICE_CM)})
                ...
                def slide_n1(top_n=20):
                    ...
                    ytd=base[(base["Year"]==selected_year)&(base["Month"]<=selected_month)]["Defect"].value_counts()
                ...
                show_fig(slide_n1(TOPN_NCR_DEFECT))
                slides_for_pdf.append({"title":"NCR - Quality Defects (CM vs YTD)","fig":slide_n1(TOPN_NCR_DEFECT)})

                st.subheader("Valid NCR Count Table")
                base_tbl=df_ncr_dash[(df_ncr_dash["Year"]==selected_year)&(df_ncr_dash["Month"]<=selected_month)&(df_ncr_dash["Is_Valid"]==True)]
                ...
                def slide_corr(top_n=12):
                    ...
                    df_cmp=df_cmp.sort_values(["CRM","NCR"],ascending=[False,False]).head(int(top_n))
                ...
                show_fig(slide_corr(TOPN_CORRELATION))
                slides_for_pdf.append({"title":"NCR vs CRM - Correlation (YTD)","fig":slide_corr(TOPN_CORRELATION)})

                st.subheader("Top Customers (FINAL)")
                def slide_cust(top_n=5):
                    base=df[(df["Is_Valid"]==True)&(df["Complaint_Category"]=="Quality")].copy()
                    CUST_COL="Customer"
                    ...
                    ytd=base[(base["Year"]==selected_year)&(base["Month"]<=selected_month)][CUST_COL].value_counts()
                ...
                try:
                    show_fig(slide_cust(TOPN_CUSTOMER))
                    slides_for_pdf.append({"title":"FINAL - Top Customers (CM vs YTD)","fig":slide_cust(TOPN_CUSTOMER)})
                except Exception as e:
