# fi_projects_tab.py
# Focused Improvement – full redesign
...
def _score_badge(score, target):
    col = C["green"] if score >= target else C["amber"] if score >= target * 0.7 else C["red"]
    st.markdown(f"""
    <div style="display:inline-flex;align-items:center;justify-content:center;
         width:80px;height:80px;border-radius:50%;background:{col};
         font-size:26px;font-weight:700;color:#fff;border:4px solid rgba(255,255,255,.3)">
      {int(score)}
    </div>""", unsafe_allow_html=True)
...
# Returns per-question met/unmet and total for the CURRENT week.
# Also returns per-week history (for ramp chart).
# ─────────────────────────────────────────────
def _compute_scores(project, team, kpi, steps, wu_rows, actions, stab, audit_records):
    """
    Returns:
        q_status: dict {qn: {"met": bool, "due": bool, "score": int, "week_due": int}}
        weekly_scores: dict {week: cumulative_score_for_that_week}
        total: int  (current cumulative score)
    """
    team     = team     or []
    steps    = steps    or []
    actions  = actions  or []
    ...
    def _parse_sp(wu):
        return _parse_json(wu.get("step_progress"), [])

    # ── evaluate each question (is it currently met?) ──
    def _met(qn):
        if qn == 1:
            return len(team) >= 1
        if qn == 2:
            return bool(team) and all(m.get("role") for m in team)
        if qn == 3:
            return bool(project.get("problem_statement")) and bool(project.get("company_kpi_link"))
        if qn == 4:
            # cost/benefit  – checked via kpi_impact field or dedicated table presence
            return bool(project.get("cost_benefit_done"))
        if qn == 5:
            return bool(kpi.get("kpi_name")) and kpi.get("baseline_value") is not None
        if qn == 6:
            return kpi.get("target_value") is not None and bool(kpi.get("target_date"))
        if qn == 7:
            subs = _parse_json(kpi.get("sub_components"), [])
            return len([s for s in subs if isinstance(s, dict)]) > 0
        if qn == 8:
            return len(steps) >= 2 and all(s.get("planned_start_week") and s.get("planned_end_week") for s in steps)
        if qn == 9:
            return bool(steps) and all(s.get("planned_end_week") for s in steps)
        if qn == 10:
            return any(_parse_sp(wu) for wu in wu_rows)
        if qn == 11:
            return any(wu.get("rca_performed") and wu.get("rca_findings") for wu in wu_rows)
        if qn == 12:
            return any(wu.get("causes_verified") for wu in wu_rows)
        if qn == 13:
            return any(wu.get("shifts_covered") and "All" in (wu.get("shifts_covered") or "") for wu in wu_rows)
        if qn == 14:
            return any(wu.get("rca_method") in {"5-Why","Fishbone","Pareto","FMEA"} for wu in wu_rows)
        if qn == 15:
            return any(wu.get("reoccurrence_description") for wu in wu_rows)
        if qn == 16:
            return any(wu.get("single_problem_analysis") and wu.get("single_problem_notes") for wu in wu_rows)
        if qn == 17:
            return bool(actions) and all(a.get("root_cause_addressed") for a in actions)
        if qn == 18:
            return any(wu.get("basic_conditions_restored") for wu in wu_rows)
        if qn == 19:
            return bool(actions) and any(a.get("target_date") for a in actions)
        if qn == 20:
            return bool(actions) and all(a.get("owner") for a in actions)
        if qn == 21:
            # action plan updated in last 2 weeks
        ...
        if qn == 23:
            return any(a.get("evidence_b64") for a in actions if a.get("status") == "Completed")
        if qn == 24:
            if len(kpi_vals) < 3:
                return False
            b = kpi.get("baseline_value", kpi_vals[0])
            t = kpi.get("target_value", b)
            return (t > b and kpi_vals[-1] > kpi_vals[-3]) or (t < b and kpi_vals[-1] < kpi_vals[-3])
        if qn == 25:
            if not kpi_vals or kpi.get("target_value") is None or kpi.get("baseline_value") is None:
                return False
            b = float(kpi["baseline_value"])
            t = float(kpi["target_value"])
            gap = abs(t - b)
            return gap > 0 and abs(kpi_vals[-1] - b) / gap >= 0.80
        if qn == 26:
            return bool(stab.get("procedures_created")) and bool(_parse_json(stab.get("procedures"), []))
        if qn == 27:
            return bool(stab.get("monitoring_in_place"))
        if qn == 28:
            return bool(stab.get("monitoring_active")) and bool(stab.get("monitoring_last_update"))
        if qn == 29:
            return bool(_parse_json(stab.get("opls"), []))
        if qn == 30:
            return bool(_parse_json(stab.get("training_matrix"), []))
        if qn == 31:
            return bool(stab.get("cil_standards_defined")) and float(stab.get("cil_audit_score") or 0) >= 90
        if qn == 32:
            return int(stab.get("five_s_rating") or 0) >= 3
        if qn == 33:
            return bool(stab.get("improvements_visible"))
        if qn == 34:
            return any((ar.get("question_scores") or {}).get("34") for ar in audit_records)
        if qn == 35:
            return any((ar.get("question_scores") or {}).get("35") for ar in audit_records)
        if qn == 36:
            return any(wu.get("meeting_held") for wu in wu_rows)
        return False
    ...
    for qn, qdata in QUESTIONS.items():
        met  = _met(qn)
        due  = cur_week >= qdata["week"]
        q_status[qn] = {
            "met":      met,
            "due":      due,
            "score":    qdata["score"] if met else 0,
            "week_due": qdata["week"],
            "text":     qdata["text"],
            "dim":      qdata["dim"],
            "max":      qdata["score"],
    ...
    # ── per-week cumulative score (for ramp chart) ──
    # For each week w, a question contributes its score if:
    #   - it is currently met  AND  its due week <= w
    # (we can't perfectly reconstruct historical met-status without per-week snapshots,
    #  so we use audit_record history where available, else project current state)
    ar_by_week = {ar["week_number"]: ar for ar in audit_records}
    weekly_scores = {}
    for w in range(1, 13):
        if w in ar_by_week:
            weekly_scores[w] = float(ar_by_week[w].get("total_score", 0))
        elif w == cur_week:
            weekly_scores[w] = total
        # weeks in the future are left out
...
def _gantt(steps, wu_rows, cur_week):
    ...
    for i, step in enumerate(steps):
        ...
        ax.barh(i, dur, left=ps - 1, height=bar_h,
                color="#d6e8f7", zorder=2)
        ax.barh(i, dur, left=ps - 1, height=bar_h,
                color="none", edgecolor="#0C5595", linewidth=1.0, zorder=3)

        # actual progress
        pct = sp_map.get(str(step.get("id", "")), 0)
        bar_col = "#1E8449" if pct == 100 else "#0C5595"
        if pct > 0:
            ax.barh(i, dur * pct / 100, left=ps - 1, height=bar_h,
                    color=bar_col, alpha=0.85, zorder=4)
            if pct >= 15:
                ax.text(ps - 1 + dur * pct / 200, i,
                        f"{pct}%", ha="center", va="center",
                        fontsize=7.5, color="white", fontweight="bold", zorder=5)

        # owner label
        if step.get("owner"):
            ax.text(pe + 0.1, i, step["owner"], ha="left", va="center",
                    fontsize=7, color="#566573", zorder=5)

        # overdue highlight
        if pct < 100 and pe < cur_week:
            ax.barh(i, dur, left=ps - 1, height=bar_h,
                    color="none", edgecolor="#DE201B", linewidth=1.5,
                    linestyle="--", zorder=5, alpha=0.7)

    # week grid
    for w in range(13):
        ax.axvline(w, color="#e5e5e5", linewidth=0.6, zorder=1)

    # current week line
    ax.axvline(cur_week - 1, color="#DE201B", linewidth=2.0,
               linestyle="--", zorder=6, alpha=0.9)
    ax.text(cur_week - 0.85, -0.55, f"W{cur_week}",
    ...
    ax.set_xticklabels([""] + [f"W{i}" for i in range(1, 13)], fontsize=8.5)
    ...
    patches = [
        mpatches.Patch(facecolor="#d6e8f7", edgecolor="#0C5595", label="Planned"),
    ...
    ax.legend(handles=patches, loc="lower right", fontsize=8,
              frameon=True, framealpha=0.95, edgecolor="#e0e0e0", ncol=4)
    fig.tight_layout(pad=0.8)
    return fig


def _kpi_chart(kpi, wu_rows):
    baseline = float(kpi.get("baseline_value") or 0)
    target   = float(kpi.get("target_value")   or 0)
    wu_sorted = sorted([w for w in wu_rows if w.get("kpi_value") is not None],
                       key=lambda x: x["week_number"])
    weeks  = [w["week_number"] for w in wu_sorted]
    values = [float(w["kpi_value"]) for w in wu_sorted]
    ...
    if weeks:
        ...
        for w, v in zip(weeks, values):
            ax.annotate(f"{v:.1f}", (w, v), textcoords="offset points",
                        ...
                        color="#0C5595", fontweight="bold")

    ax.set_xlim(0.5, 12.5)
    all_vals = values + [baseline, target]
    spread = max(abs(target - baseline), 1)
...
def _ramp_chart(weekly_scores, cur_week):
    ...
    if weekly_scores:
        ws = sorted(weekly_scores.keys())
        wv = [weekly_scores[w] for w in ws]
        ...
        for w, v in zip(ws, wv):
            ax.annotate(f"{int(v)}", (w, v), textcoords="offset points",
...
# ─────────────────────────────────────────────
# ANALYSIS TOOLS – 5 WHY
# ─────────────────────────────────────────────
def _five_why_chart(problem, whys):
    """Renders a vertical 5-Why chain as a clean PNG."""
...
def render_fi_projects_tab(supabase, role, pillar, name):
            ...
            with st.form("fi_create_form"):
                ...
                if st.form_submit_button("Create", type="primary"):
                    if n_name and n_problem:
                        ...
                        st.session_state["fi_creating"] = False
                        st.rerun()
                    else:
    ...
    sec = st.radio("", SECTIONS, horizontal=True, key="fi_section",
    ...
    if sec == "Overview":
        ...
        score_col = "green" if total_score >= tgt_this_week else "orange" if total_score >= tgt_this_week * 0.7 else "red"
        h3.markdown(f"**Score** &nbsp; <span style='font-size:26px;font-weight:700;color:{'#1E8449' if score_col=='green' else '#D68910' if score_col=='orange' else '#DE201B'}'>{int(total_score)}</span> / 100 &nbsp; *(target {tgt_this_week})*", unsafe_allow_html=True)

        # KPI cards
        if kpi:
            sub_c = _parse_json(kpi.get("sub_components"), [])
            sub_c = [s for s in sub_c if isinstance(s, dict)]
            ...
            b = float(kpi.get("baseline_value") or 0)
            t = float(kpi.get("target_value")   or 0)
            gap = abs(t - b)
            prog = abs(kpi_cur - b) / gap * 100 if gap > 0 else 0
            ...
            with cols[3]: _card("Progress",  f"{prog:.0f}%", "Toward target",
                                "#1E8449" if prog >= 80 else "#D68910" if prog >= 40 else "#DE201B")
            st.progress(min(int(prog), 100))

            # KPI trend
            fig_kpi = _kpi_chart(kpi, wu_rows)
            st.pyplot(fig_kpi)
            plt.close(fig_kpi)

        # score ramp
        st.markdown("##### Score vs Target Ramp")
        fig_ramp = _ramp_chart(weekly_scores, cw)
        st.pyplot(fig_ramp)
        plt.close(fig_ramp)
        ...
        if team:
            st.markdown("##### Team")
            df_t = pd.DataFrame([{"Name": m["member_name"], "Role": m.get("role",""), "Department": m.get("department","")} for m in team])
        ...
        if can_edit:
            with st.expander("Edit Project Details"):
                ...
                e_sec = ea1.selectbox("Section", list(PLANT_SECTIONS.keys()),
                    index=list(PLANT_SECTIONS.keys()).index(_cur_sec) if _cur_sec in PLANT_SECTIONS else 0, key="fi_e_sec")
                _em = PLANT_SECTIONS.get(e_sec, [])
                _cur_mach = _cur_area.split(" — ")[1] if " — " in _cur_area else ""
                e_mach = ea2.selectbox("Machine", ["— All —"] + _em,
                    index=(_em.index(_cur_mach)+1) if _cur_mach in _em else 0, key="fi_e_mach") if _em else None
                ...
                with st.form("fi_edit_project"):
                    ...
                    if st.form_submit_button("Save"):
                        supabase.table("fi_projects").update({
                            "project_name": e_name, "target_area": e_area,
                            "problem_statement": e_problem, "company_kpi_link": e_kpi_lnk,
                            "launch_date": str(e_launch), "expected_completion_date": str(e_end),
                        }).eq("id", pid).execute()
            ...
            with st.expander("Manage Team"):
                with st.form("fi_add_team"):
                    ...
                    t_name = tm1.text_input("Name *")
                    t_role = tm2.selectbox("Role", TEAM_ROLES)
                    t_dept = tm3.text_input("Department")
                    if st.form_submit_button("Add"):
                        if t_name:
                            supabase.table("fi_project_team").insert({"project_id":pid,"member_name":t_name,"role":t_role,"department":t_dept,"contribution_target":""}).execute()
                            st.rerun()
                if team:
            ...
            with st.expander("KPI & KAI Setup"):
                ...
                _cur_cat  = kpi.get("kpi_category", "OEE Improvement")
                ...
                _def_unit = KPI_TREE[kpi_cat]["unit"]
                _kai_opts = KPI_TREE[kpi_cat]["kais"]
                _ex_subs  = _parse_json(kpi.get("sub_components"), [])
                _ex_subs  = [s for s in _ex_subs if isinstance(s, dict)]
                _ex_names = [s.get("name","") for s in _ex_subs]
                _def_kais = [k for k in _ex_names if k in _kai_opts]
                ...
                with st.form("fi_kpi_form"):
                    ...
                    _cur_unit = kpi.get("unit", _def_unit)
                    ...
                    if sel_kais:
                        st.divider()
                        _sub_map = {s.get("name",""): s for s in _ex_subs}
                        _tnames  = [""]+[m["member_name"] for m in team]
                        rh = st.columns([3,1.5,1.2,1.2,2])
                        for h, t_h in zip(rh, ["KAI","Unit","Baseline","Target","Owner"]):
                            h.caption(t_h)
                        for si, kai in enumerate(sel_kais):
                            ex = _sub_map.get(kai, {})
                            r = st.columns([3,1.5,1.2,1.2,2])
                            r[0].markdown(f"**{kai}**")
                            _ku = ex.get("unit", _def_unit)
                            _ko = ex.get("owner","")
                            kai_rows.append({
                                "name": kai,
                                "unit":     r[1].selectbox("U", UNITS, index=UNITS.index(_ku) if _ku in UNITS else 0, key=f"ku_{si}", label_visibility="collapsed"),
                    ...
                    if st.form_submit_button("Save KPI & KAIs", type="primary"):
                        payload = {"project_id":pid,"kpi_name":kpi_cat,"unit":k_unit,
                                   "kpi_category":kpi_cat,"baseline_value":k_base,
                                   "target_value":k_tgt,"target_date":str(k_date),
        ...
        if can_edit:
            with st.expander("Add / Remove Steps"):
                with st.form("fi_add_step"):
                    s1, s2 = st.columns(2)
                    s_name  = s1.text_input("Step Name *")
                    s_desc  = s2.text_input("Description")
                    ...
                    if st.form_submit_button("Add Step"):
                        if s_name:
                            supabase.table("fi_project_steps").insert({"project_id":pid,"step_name":s_name,"description":s_desc,"planned_start_week":int(s_start),"planned_end_week":int(s_end),"owner":s_owner,"sort_order":len(steps)}).execute()
                            st.rerun()
                if steps:
    ...
    # ══════════════════════════════════════════════════
        ...
        with st.expander(f"KPI Reading — Week {sel_week}", expanded=True):
            if not kpi:
                st.info("Set up your KPI in Overview first.")
            else:
                with st.form(f"fi_kpi_w{sel_week}"):
                    wk1, wk2 = st.columns(2)
                    kpi_val_in = wk1.number_input(
                        f"{kpi.get('kpi_name','')} value ({kpi.get('unit','')})",
                        value=float(wu.get("kpi_value", kpi.get("baseline_value",0)) or 0))
                    coll_by = wk2.multiselect("Collected by", [m["member_name"] for m in team],
        ...
        with st.expander(f"Step Progress — Week {sel_week}", expanded=True):
            ...
            if not due_steps:
                st.info("No steps due yet this week.")
            else:
                sp_list = _parse_json(wu.get("step_progress"), [])
                sp_map2 = {sp["step_id"]: sp for sp in sp_list if isinstance(sp, dict)}
                PCT = ["0%","25%","50%","75%","100%"]
                with st.form(f"fi_sp_w{sel_week}"):
                    new_sp = []
                    chunks2 = [due_steps[i:i+3] for i in range(0,len(due_steps),3)]
        ...
        with st.expander(f"Meeting Log — Week {sel_week}"):
            with st.form(f"fi_mtg_w{sel_week}"):
                ...
                if team:
                    ...
                    st.caption(f"Attendance: {att_rate:.0f}%  ({len(attendees)}/{len(team)})")
                if st.form_submit_button("Save"):
        ...
        if sel_week >= 3:
            with st.expander(f"Root Cause Analysis — Week {sel_week}"):
                with st.form(f"fi_rca_w{sel_week}"):
                    ...
                    if sel_week >= 7:
                        st.divider()
                        st.caption("Reoccurrence Analysis")
                    ...
                    if st.form_submit_button("Save"):
                        ...
                        if rca_done:
                            upd.update({"rca_method": rca_method, "rca_findings": rca_findings,
                                        "causes_verified": causes_ver, "causes_verification_method": causes_meth})
                        if sel_week >= 7:
                            upd.update({"reoccurrence_before":reoc_b,"reoccurrence_description":reoc_d,
                                        "reoccurrence_prevention":reoc_p,"single_problem_analysis":s_pa,"single_problem_notes":s_n})
                        _save_wu(upd)
    ...
    # ══════════════════════════════════════════════════
        ...
        if tool == "5-Why":
            ...
            st.caption("Start from the problem and ask 'Why?' five times to reach the root cause.")
            prob = st.text_area("Problem Statement", height=70,
                value=selected_project.get("problem_statement",""), key="fy_problem")
            whys = []
            for i in range(5):
                whys.append(st.text_input(f"Why {i+1}", key=f"fy_{i}"))
            root = whys[-1] if whys[-1].strip() else (next((w for w in reversed(whys) if w.strip()), ""))

            if st.button("Generate Chart", type="primary", key="fy_gen"):
                active_whys = [w for w in whys if w.strip()]
                if prob and active_whys:
                    fig_fy = _five_why_chart(prob, active_whys)
            ...
            st.caption("Enter causes per category. Standard categories pre-filled — customise as needed.")
            prob_fb = st.text_input("Effect / Problem", value=selected_project.get("problem_statement","")[:80], key="fb_prob")
            ...
            for ci, cat in enumerate(default_cats):
                with cat_cols[ci % n_cols]:
                    raw = st.text_area(cat, height=100, key=f"fb_{cat}",
                                       placeholder="One cause per line…")
                    cats_in[cat] = [l.strip() for l in raw.split("\n") if l.strip()]

            if st.button("Generate Fishbone", type="primary", key="fb_gen"):
...
""")
    ...
    # ══════════════════════════════════════════════════
        ...
        overdue   = [a for a in actions if a.get("status") == "Overdue" or
                     (a.get("target_date") and _safe_date(a["target_date"]) and
                      _safe_date(a["target_date"]) < date.today() and a.get("status") != "Completed")]

        ac1, ac2, ac3, ac4 = st.columns(4)
        ac1.metric("Open",       len(open_a))
        ac2.metric("In Progress",len(in_prog), delta=None)
        ac3.metric("Completed",  len(completed))
        ac4.metric("Overdue",    len(overdue),  delta=f"-{len(overdue)}" if overdue else None, delta_color="inverse")
            ...
            with col:
                ...
                for a in [x for x in actions if x.get("status") == status]:
                    is_od = (a.get("target_date") and _safe_date(a["target_date"]) and
                             _safe_date(a["target_date"]) < date.today() and status != "Completed")
                    border = "#DE201B" if is_od else {"Open":"#0C5595","In Progress":"#D68910","Completed":"#1E8449"}.get(status,"#BDC3C7")
                    st.markdown(f"""
                    <div style="border-left:4px solid {border};background:#f9f9f9;
                         border-radius:6px;padding:10px 12px;margin-bottom:8px;font-size:13px">
                      <b>{a.get('description','')[:60]}</b><br>
                      <span style="color:#566573;font-size:11px">
                        👤 {a.get('owner','—')} &nbsp; 📅 {str(a.get('target_date',''))[:10]}
                        {' &nbsp; ⚠️ OVERDUE' if is_od else ''}
                      </span>
                    </div>""", unsafe_allow_html=True)
        ...
        if can_edit:
            with st.expander("Add New Action"):
                with st.form("fi_add_action"):
                    ...
                    a_desc = na1.text_area("Description *", height=70)
                    a_rc   = na2.text_area("Root Cause Addressed", height=70)
                    ...
                    if st.form_submit_button("Add Action"):
                        if a_desc:
                            supabase.table("fi_actions").insert({
                                "project_id":pid,"description":a_desc,"root_cause_addressed":a_rc,
                                "owner":a_own,"target_date":str(a_date),"status":"Open",
                                "created_week":cw,
                                "evidence_b64":_b64(a_ev) if a_ev else None,
                                "evidence_filename":a_ev.name if a_ev else None,
                            }).execute()
                            st.rerun()
    ...
    # ══════════════════════════════════════════════════
        ...
        st.caption("Sections unlock as the project progresses. Ensuring gains are held.")
        stab_v = stab or {}

        def _stab_save(d):
            if stab_v.get("id"):
                supabase.table("fi_stabilisation").update(d).eq("id", stab_v["id"]).execute()
            else:
                supabase.table("fi_stabilisation").insert({"project_id":pid, **d}).execute()
            st.rerun()
        ...
        with st.expander(f"{'✅' if q_status[27]['met'] else '⬜'} Monitoring Systems {'· UNLOCKS W7' if cw<7 else ''}"):
            with st.form("fi_mon"):
                ...
                mon_types = st.multiselect("Types", MONITORING_TYPES,
                    default=[x.strip() for x in (stab_v.get("monitoring_types","") or "").split(",") if x.strip() in MONITORING_TYPES])
                mon_act   = st.checkbox("Actively used?", value=bool(stab_v.get("monitoring_active")))
                mon_date  = st.date_input("Last update",
                    value=date.fromisoformat(str(stab_v["monitoring_last_update"])) if stab_v.get("monitoring_last_update") else date.today())
        ...
        with st.expander(f"{'✅' if q_status[29]['met'] else '⬜'} OPLs & Training {'· UNLOCKS W10' if cw<10 else ''}"):
            with st.form("fi_opls"):
                ...
                st.divider()
                st.caption("Training Matrix")
                ex_tm  = _parse_json(stab_v.get("training_matrix"), [])
                new_tm = []
                for mi, mem in enumerate(team):
                    mn = mem["member_name"]
                    em = next((x for x in ex_tm if x.get("member")==mn),{})
                ...
                if st.form_submit_button("Save"):
                    _stab_save({"opls":json.dumps(new_opls),"training_matrix":json.dumps(new_tm)})
        ...
        with st.expander(f"{'✅' if q_status[26]['met'] else '⬜'} Procedures {'· UNLOCKS W10' if cw<10 else ''}"):
            with st.form("fi_proc"):
                ...
                ex_procs  = _parse_json(stab_v.get("procedures"), [])
                n_p       = st.number_input("Number of procedures", min_value=0, max_value=10, value=max(0,len(ex_procs)))
                new_procs = []
                for pi in range(int(n_p)):
                    ep = ex_procs[pi] if pi < len(ex_procs) else {}
                ...
                if st.form_submit_button("Save"):
                    _stab_save({"procedures_created":proc_done,"procedures":json.dumps(new_procs)})
    ...
    # ══════════════════════════════════════════════════
        ...
        tgt = TARGET_RAMP.get(cw, 100)
        gap_n = total_score - tgt
        ...
        ah2.metric("Target W" + str(cw), tgt)
        ...
        # ramp chart
        fig_r2 = _ramp_chart(weekly_scores, cw)
        st.pyplot(fig_r2)
        plt.close(fig_r2)
        ...
        for ci, (dim, qs) in enumerate(DIMS.items()):
            ...
            pct      = achieved / possible * 100 if possible else 0
        ...
        for qn, qs in q_status.items():
            if qs["due"]:
                status_s = "✅ Met" if qs["met"] else "🔴 Not Met"
            else:
                status_s = "⏳ Not Due"
            rows.append({"Q": qn, "Question": qs["text"], "Dim": qs["dim"],
                         "Weight": qs["max"], "Due W": qs["week_due"],
                         "Status": status_s, "Score": f"{qs['score']}/{qs['max']}"})
        ...
        with st.form("fi_audit_form"):
            ...
            t_member = au1.text_input("Team member tested")
            u_pass   = au2.checkbox("Explained correctly?")
            ...
            if st.form_submit_button("Save Audit Record"):
                ...
                if u_pass:
                    qs_save["34"] = True
                    qs_save["35"] = True
                ar_payload = {"project_id":pid,"week_number":cw,"question_scores":json.dumps(qs_save),
                              "total_score":total_score,"team_understanding_tested":bool(t_member),
                              "member_tested":t_member,"understanding_pass":u_pass,
                              "auditor_notes":a_notes,"audited_by":name}
                ex_ar = next((ar for ar in audit_records if ar["week_number"]==cw), None)
                if ex_ar:
                    supabase.table("fi_audit_records").update(ar_payload).eq("id",ex_ar["id"]).execute()
                else:
                    supabase.table("fi_audit_records").insert(ar_payload).execute()
                st.success("Saved")
                st.rerun()
