...
def _generate_project_pdf(project, team, kpi, steps, weekly_updates, actions, stab, audit_records, scores, total_score):
    ...
    TARGET_RAMP_LOCAL = {1:12,2:15,3:24,4:33,5:56,6:58,7:77,8:79,9:79,10:85,11:100,12:100}
        ...
        if current_week >= 5:
            with st.expander("💰 Cost / Benefit", expanded=False):
                ...
                with st.form("fi_cost_form"):
                    ...
                    cost_est  = cb1.number_input("Estimated Project Cost (K SAR)", value=float(cost_vals.get("estimated_cost",0) or 0))
                    cost_save = cb2.number_input("Estimated Annual Savings (K SAR/yr)", value=float(cost_vals.get("estimated_savings",0) or 0))
    ...
    with subtabs[2]:
        ...
        if not kpi:
            ...
            if cost:
                ...
                cb_c1.metric("Estimated Cost",         f"{cost.get('estimated_cost',0):.1f} K SAR")
                cb_c2.metric("Estimated Annual Savings",f"{cost.get('estimated_savings',0):.1f} K SAR/yr")
